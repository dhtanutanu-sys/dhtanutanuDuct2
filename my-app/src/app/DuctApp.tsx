...
const DuctApp = () => {
  useEffect(() => {
    ...
    function getDefaultFittings() {
        return {
            ...
            '可変角度エルボ': [
             ...
             'レジューサー': [
                { id: 'reducer-100-100', name: 'D100-100', diameter: 100, diameter2: 100, length: 150, visible: true },
            ],
            'ダンパー': [
                { id: 'damper-100-100', name: 'VD100 L100', diameter: 100, length: 100, visible: false },
            ],
        };
    }
    ...
    // パレット開閉
    // =================================================================================
    document.getElementById('toggle-palette-btn').addEventListener('click', () => {
  ...
  return (
    <>
      {/* 部品パレット (サイドバー) */}
      <aside id="palette" className="w-full md:w-64 bg-white shadow-lg p-4 overflow-y-auto order-first md:order-last">
        <div className="mb-6">
          <div className="space-y-2">
            <div>
              <input type="text" id="system-name" defaultValue="SA-1" className="w-full p-2 border rounded-md focus:ring-2 focus:ring-indigo-500" placeholder="系統名" />
            </div>
            <div>
              <input type="number" id="custom-diameter" defaultValue="100" step="25" min="25" className="w-full p-2 border rounded-md focus:ring-2 focus:ring-indigo-500" placeholder="直径 (mm)" />
            </div>
            <button id="add-custom-straight" className="w-full bg-indigo-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-indigo-700 transition-colors">
              直管を追加
            </button>
          </div>
        </div>
        <div className="mb-6 border-t pt-4">
          <button id="manage-fittings-btn" className="w-full bg-gray-200 text-gray-800 font-semibold py-2 px-4 rounded-md hover:bg-gray-300 transition-colors">継手管理</button>
        </div>
        <div id="palette-items" className="grid grid-cols-2 gap-4">
          {/* パレットアイテムはJSで動的に生成 */}
        </div>
      </aside>

      {/* メインコンテンツ */}
      <main className="flex-1 flex flex-col relative print-area">
        {/* ツールバー */}
        <header className="bg-white/80 backdrop-blur-sm shadow-md p-2 flex items-center justify-between z-10">
          <div className="flex items-center flex-wrap gap-x-1 md:gap-x-2 gap-y-1">
            <button id="toggle-palette-btn" title="パレットを開閉" className="p-2 rounded-md hover:bg-gray-200 md:hidden">
            ...
            <button id="undo-btn" title="元に戻す" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="redo-btn" title="やり直す" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="measure-tool" title="2点間計測" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="zoom-in" title="ズームイン" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="zoom-out" title="ズームアウト" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="reset-view" title="ビューをリセット" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="clear-canvas" title="キャンバスをクリア" className="p-2 rounded-md hover:bg-gray-200 text-red-500">
            ...
            <button id="screenshot-btn" title="スクリーンショット" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="share-btn" title="共有" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="print-btn" title="印刷" className="p-2 rounded-md hover:bg-gray-200"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9V2h12v7" /><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2" /><path d="M6 14h12v8H6z" /></svg></button>
          </div>
        </header>
        ...
        <div className="flex-1 bg-gray-200 relative overflow-hidden">
          ...
          <div id="info-overlay" className="absolute top-2 left-2 bg-white/70 backdrop-blur-sm p-2 rounded-md text-sm pointer-events-none"></div>
          {/* コンテキストメニュー */}
          <div id="context-menu" className="absolute bg-white shadow-lg rounded-md p-1 flex items-center space-x-1 z-20 hidden">
            <button id="rotate-btn" title="回転 (R)" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="flip-btn" title="反転" className="p-2 rounded-md hover:bg-gray-200">
            ...
            <button id="delete-btn" title="削除 (Delete)" className="p-2 rounded-md hover:bg-gray-200 text-red-600">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
            </button>
            <button id="disconnect-btn" title="接合を解除" className="p-2 rounded-md hover:bg-gray-200">
        ...
        <div id="notification" className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-gray-800 text-white px-4 py-2 rounded-md shadow-lg transition-opacity duration-300 opacity-0 pointer-events-none z-30">
        </div>
      </main>

      {/* 継手管理モーダル */}
      <div id="fittings-modal" className="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
        <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-6xl max-h-[90vh] flex flex-col">
          <div className="flex justify-between items-center border-b pb-3 mb-4">
            ...
            <button id="close-modal-btn" className="p-2 rounded-full hover:bg-gray-200">&times;</button>
          </div>
          ...
          <div className="mt-4 border-t pt-4 flex justify-end">
            <button id="save-fittings-btn" className="bg-indigo-600 text-white font-semibold py-2 px-6 rounded-md hover:bg-indigo-700 transition-colors">保存</button>
          </div>
        </div>
      </div>

      {/* 汎用モーダル (確認/入力) */}
      <div id="modal" className="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
        <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-sm">
          <h2 id="modal-title" className="text-xl font-bold mb-4"></h2>
          <div id="modal-body"></div>
          <div id="modal-footer" className="mt-4 flex justify-end space-x-2"></div>
        </div>
      </div>
    </>
  );
